...
class QRLoginDialog(tk.Toplevel):
    ...
    def _on_qr_generated(self, ok: bool, tokenimage: str,
        ...
        photo, perr = self._make_qr_photo(payload)
        if photo is None:
            # 图像渲染彻底失败 — 给出可读提示 + 网页扫码兜底, 不阻断其它逻辑。
            self._set_status(
                "二维码图像无法显示(%s)，可点【改用网页扫码】。" % perr, "#dc2626")
            return
        self._photo = photo
    ...
    def _make_qr_photo(self, payload_b64: str):
        """把 base64 PNG 渲染成可显示的 PhotoImage。

        优先用 Pillow(可平滑缩放); 没装 Pillow 时回退到 Tk 8.6 原生 PNG 支持
        (tk.PhotoImage(data=...)), 因此**不再强制依赖 Pillow**。
        返回 (photo 或 None, 错误信息)。
        """
        # 路径一: Pillow (若已安装)
        if _PIL_OK:
            try:
        ...
        # 路径二: Tk 原生 PNG (Tk 8.6+), data= 接受 base64 字符串
        try:
            photo = tk.PhotoImage(master=self, data=payload_b64)
            # 二维码原图较小, 整数放大便于扫描 (失败也无妨)
            try:
                zoom = max(1, _QR_DISPLAY_SIZE[0] // max(1, photo.width()))
                if zoom > 1:
            ...
            return None, "PIL:%s; Tk:%s" % (pil_err, e)
