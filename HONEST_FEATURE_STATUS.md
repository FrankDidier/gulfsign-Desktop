...
## 10. 户主代申请 (2026-06-06, 对照对方内部聊天截图复刻)

对方一张内部聊天截图揭示了其真正打法 (与本仓库 `ph3_status6` 探针报告互相印证):

> "走家庭居民申请 = 相当于你申请家庭签约 / 微信上面把那个接口隐藏了 /
>  如果你是户主, 你就可以给自己下面的人申请 / 转成居民申请这样隐藏得好啊"

### 10.1 机制对齐 (两边证据吻合)

- 本仓库探针早已实测: **居民申请(STATUS=6) 只能由居民端 (健康卡/微信) 经
  `insertJtysqy` 产生**; 公卫医生端只能产出 5, 且 5 无法被确认成已签约。
- 截图证实居民端入口 = 微信"我的健康卡"里**户主对全户成员的家庭代申请** (隐藏入口)。
- 在本平台上, 这等价于: **家庭成员的健康卡都绑在同一个户主 openid 下**
  (`newlist` 一次性返回全部), 而 `insertJtysqy` / `editqr` 全程使用户主 openid
  对每张成员卡发起 + 确认。→ **绑定数量按"户"算, 不按"人"算**, 这解释了
  "对方哪来那么多绑定" —— 他们不需要逐人绑卡。

### 10.2 本轮新增 (REAL, 已离线验证)

| 改动 | 说明 |
|------|------|
| `sign_engine.HouseholdSignResult` | 整户结果聚合 (total/confirmed/created/already_signed/failed) |
| `sign_engine.list_household_members()` | 拉取户主 openid 名下全部成员卡 (newlist) |
| `sign_engine.process_household()` | 户主代申请: 对全户成员逐人 `process_card_full`, 支持 `relation_filter` / 进度 / 中止 |
| `.dbg/household_unit_test.py` (10) | FakeHC 验证聚合统计 / 过滤 / 边界 / **全程同一户主 openid** |
| `.dbg/household_functional_test.py` (2) | 真 socket 有状态仿真平台: 真实发出 `insertJtysqy`+`editqr`, 断言**每次都携带户主 openid**, 服务端状态真被推进 1→6→0 |

### 10.3 验证边界 (诚实声明)

- **已离线验证**: 请求形状、户主 openid 全程不变、状态机 1→6→0、聚合统计正确
  (真 socket 仿真, 共 12 条新测试全绿; 全量回归 249 条全绿)。
- **尚未在生产户上联机验证**的关键假设 (需一个名下绑有家庭成员的真实户主 openid):
  1. 用户主 openid 对**同户其他成员** `insertJtysqy` 是否被生产服务端接受写 6;
  2. 户内成员是否**免逐人人脸** (仅验户主) —— 这是规模化的命门;
  3. 确认后居民微信是否照常收到"签约成功通知"。
  → 截图强烈指向以上三条成立 (对方正以此规模化), 但本工具坚持: **未联机证实的,
    不当成已证实**。给一个有家庭成员的户主 openid 即可在 1 户上闭环验证。
